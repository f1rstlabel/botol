...
class Peminjaman extends CI_Controller {
    ...
    public function proses_pinjam() {
        ...
        if (!empty($barang_ids)) {
            ...
            $ids = [$barang_id]; 
            $jumlah_default = $jumlah_pinjam; 
        } else {
            $this->session->set_flashdata('message', 'Tidak ada barang yang dipilih.');
            redirect('peminjaman/create');
            return;
        }

        if (empty($peminjam) || empty($tanggal_pinjam) || empty($tanggal_kembali) || empty($departemen)) {
            $this->session->set_flashdata('message', 'Semua field harus diisi.');
            redirect('peminjaman/form_multiple'); 
        }
        ...
        foreach ($ids as $id) {
            $data = [
                ...
                'jumlah_pinjam' => $jumlah_default,
                'departemen' => $departemen,
                'status_pengembalian' => 0
            ];

            // Cek stok cukup (optional jika hanya tampilkan status OK)
            if (!$this->Peminjaman_model->is_qty_available($id, $jumlah_default)) {
                $this->session->set_flashdata('message', 'Stok barang ID ' . $id . ' tidak mencukupi.');
                redirect('peminjaman/create');
                return;
            }
        ...
        $this->session->set_flashdata('message', 'Peminjaman berhasil diproses.');
        redirect('peminjaman');
}
